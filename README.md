...
- **No `unsafe`.**
- **Fail-soft by design.** Extensively fuzzed and pinned by [`keksbruch`](crates/keksbruch), the differential test harness, and its 40+ parser matrix — designed not to panic on, or echo injection bytes from, malformed input. Thoroughly tested, but provided without warranty (see [License & Disclaimer](#license--disclaimer)).
