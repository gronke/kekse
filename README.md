...
There is no cookie jar, no signing or encryption, and no date handling — a lifetime is `Max-Age` seconds, never an `Expires` date — so it pulls in neither `time` nor `chrono`.
