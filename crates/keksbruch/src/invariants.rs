...
/// No parsed name or value may carry an injection byte (`;`, CR, LF, NUL) — the
/// security invariant: attacker bytes can never survive the decode into a pair
/// that would split or smuggle a header downstream.
pub fn assert_no_injection_echo(wire: &str) {
    for (name, value) in parse_pairs(wire) {
        for field in [name, value.as_ref()] {
            assert!(
                !field.bytes().any(|b| matches!(b, b';' | b'\r' | b'\n' | 0)),
