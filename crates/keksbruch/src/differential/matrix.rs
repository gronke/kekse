...
fn consensus(row: usize, columns: &[Column]) -> Option<String> {
    ...
    for column in columns {
        ...
        match &column.cells[row] {
            ParseOutcome::NotApplicable | ParseOutcome::Skipped => continue,
...
pub fn render(scenarios: &[Scenario], columns: &[Column], versions: &[String]) -> String {
    ...
    out.push_str(
        "Legend: `[n=v, …]` parsed pairs · `∅` parsed to nothing · `✗reject` whole header/cookie \
         rejected (fail-hard) · `n/a` direction not handled · `SKIP` parser unavailable · \
         `PANIC` adapter panicked. **Bold** = differs from the real-world *consensus*. \
    ...
    out.push_str(
        "The `cookie` crate runs in percent-decoding (`*_encoded`) mode so its values are \
         ...
         exposes no general iterator, so its accepted pairs are enumerated by name. Go \
